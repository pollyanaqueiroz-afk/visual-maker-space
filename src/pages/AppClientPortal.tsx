...
import { CheckCircle2, Circle, Lock, AlertTriangle, ExternalLink, MessageSquare, Star } from 'lucide-react';
...
function PortalContent() {
  ...
  const { data: cliente, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase.from('app_clientes').select('*').eq('portal_token', token!).single();
  ...
  const { data: fases = [] } = useQuery({
    queryKey: ['portal-fases', cliente?.id],
    enabled: !!cliente,
    queryFn: async () => {
      const { data, error } = await supabase.from('app_fases').select('*').eq('cliente_id', cliente!.id).order('numero');
      if (error) throw error;
      return data;
    },
  });
  ...
  const { data: checklist = [] } = useQuery({
    queryKey: ['portal-checklist', cliente?.id],
    enabled: !!cliente,
    queryFn: async () => {
      const { data, error } = await supabase.from('app_checklist_items').select('*').eq('cliente_id', cliente!.id).order('ordem');
      if (error) throw error;
      return data;
    },
  });
  ...
  const { data: formulario } = useQuery({
    queryKey: ['portal-formulario', cliente?.id],
    enabled: !!cliente,
    queryFn: async () => {
      const { data, error } = await supabase.from('app_formulario').select('*').eq('cliente_id', cliente!.id).single();
      if (error) throw error;
      return data;
    },
  });
  ...
  const { data: prereqs } = useQuery({
    queryKey: ['portal-prereqs', cliente?.id],
    enabled: !!cliente,
    queryFn: async () => {
      const { data, error } = await supabase.from('app_prerequisitos').select('*').eq('cliente_id', cliente!.id).single();
      if (error) throw error;
      return data;
    },
  });
  ...
  const saveForm = useMutation({
    mutationFn: async () => {
      ...
      if (error) throw error;
      // Mark form checklist items as done
      if (complete) {
  ...
  if (isLoading) return <div className="min-h-screen bg-[#0F172A] flex items-center justify-center text-white/60">Carregando...</div>;
  if (!cliente) return <div className="min-h-screen bg-[#0F172A] flex items-center justify-center text-white/60">Portal não encontrado</div>;
      ...
      <div className="max-w-2xl mx-auto px-4 py-8 space-y-8">
        ...
        {cliente.fase_atual === 8 ? (
          <div className="text-center space-y-4 py-12">
            <h1 className="text-4xl font-extrabold">🎉 SEU APP ESTÁ PUBLICADO!</h1>
            <p className="text-white/70 text-lg">Parabéns, {cliente.nome}! Seu app da {cliente.empresa} está disponível nas lojas.</p>
          </div>
        ) : (
          <div className="space-y-4">
            <h1 className="text-2xl font-bold">Olá, {cliente.nome}! 👋</h1>
            ...
            <div className="flex flex-wrap gap-4 text-sm text-white/60">
              ...
              {cliente.prazo_estimado && <span>🗓️ Publicação estimada: {format(new Date(cliente.prazo_estimado), 'dd/MM/yyyy')}</span>}
            </div>
        ...
        {prereqs?.cnpj_bloqueado && (
          <Card className="bg-yellow-500/10 border-yellow-500/30 p-4 text-yellow-200">
            ...
            <p className="text-sm mt-1 text-yellow-200/70">A Apple exige CNPJ do tipo ME ou LTDA para publicar apps.</p>
            {cliente.responsavel_nome && (
              <p className="text-sm mt-2">Seu responsável: {cliente.responsavel_nome}</p>
            )}
          </Card>
        )}
          ...
          <Card className="bg-[#1E293B] border-white/10 p-6 space-y-4">
            ...
            {hasClientAction ? (
              ...
              <div className="text-center py-4 text-white/50">
                ...
                {fases[cliente.fase_atual] && (
                  <p className="text-xs mt-2">Estimativa: {fases[cliente.fase_atual]?.duracao_dias_estimada || '?'} dias</p>
                )}
              </div>
            )}
          </Card>
        ...
        {cliente.fase_atual === 4 && (
          <Card className="bg-[#1E293B] border-white/10 p-6 space-y-4">
            ...
            <div className="space-y-3">
              ...
              <div>
                <Label className="text-white/70">Descrição curta * <span className="text-xs">({formData.descricao_curta.length}/80)</span></Label>
              ...
              <div>
                <Label className="text-white/70">Descrição completa <span className="text-xs">({formData.descricao_longa.length}/4000)</span></Label>
        ...
        <Card className="bg-[#1E293B] border-white/10 p-6">
          ...
          <div className="space-y-2 text-sm text-white/60">
            ...
            {cliente.prazo_estimado && (
              <p>Prazo estimado: <span className="text-white">{format(new Date(cliente.prazo_estimado), 'dd/MM/yyyy')}</span></p>
            )}
          </div>
        </Card>
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-2">
            {fases.filter(f => f.status === 'concluida').map(f => (
              <Badge key={f.id} className="bg-green-500/20 text-green-400 px-3 py-1">
                ✓ {f.nome}
              </Badge>
            ))}
