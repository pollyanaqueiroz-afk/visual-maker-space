...
import {
  ...
  ChevronRight, CheckCircle2, BarChart3, Users,
} from 'lucide-react';
...
function generateMockInsights() {
  ...
  return {
    kpis: { total_suggestions: 14, critical_bugs: 3, mrr_at_risk: 243500, tickets_this_week: 28 },
    insights,
    componentChart,
...
export default function TicketsInsightsTab() {
      ...
      <Tabs defaultValue="insights" className="space-y-4">
        ...
        <TabsContent value="insights" className="space-y-3">
          {insights.map(insight => {
            ...
            return (
              <Card key={insight.id} className={`cursor-pointer transition-all hover:shadow-md ${isSelected ? 'ring-2 ring-primary/30' : ''}`} onClick={() => setSelectedInsight(isSelected ? null : insight.id)}>
                <CardContent className="p-4">
                      ...
                      {isSelected && (
                        <div className="mt-3 pt-3 border-t space-y-2">
                          ...
                          <div className="flex gap-2 mt-2">
                            <Button size="sm" className="h-7 text-xs gap-1"><Lightbulb className="h-3 w-3" /> Sugerir Melhoria ao Produto</Button>
                            <Button size="sm" variant="outline" className="h-7 text-xs gap-1"><BookOpen className="h-3 w-3" /> Criar Artigo na Base de Conhecimento</Button>
                          </div>
                        </div>
                      )}
                    </div>
                  ...
                  <div key={item.id} className="flex items-center gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-3 mt-0.5 text-[10px] text-muted-foreground">
                        ...
                        <Badge className={`text-[9px] px-1 py-0 border-0 ${SEVERITY_BADGE[item.severity]}`}>{item.severity}</Badge>
                      </div>
                    </div>
                  </div>
                );
              ...
              <div className="h-72">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={componentChart} layout="vertical" margin={{ left: 10, right: 20 }}>
                    ...
                    <Tooltip contentStyle={{ fontSize: 12, borderRadius: 8 }} formatter={(value: number) => [`${value} tickets`, 'Quantidade']} />
                    <Bar dataKey="count" radius={[0, 4, 4, 0]}>
                      {componentChart.map((entry, i) => <Cell key={i} fill={entry.fill} />)}
                    </Bar>
                  </BarChart>
                </ResponsiveContainer>
              </div>
