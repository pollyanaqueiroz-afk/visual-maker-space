...
-- Update the criar_fases_cliente function to use new phase 3 items
...
BEGIN
  ...
  -- FASE 3 - Criação e Submissão (only 2 items now)
  INSERT INTO app_checklist_items (cliente_id, fase_numero, texto, descricao, ator, tipo, ordem)
  VALUES
...
-- Delete old phase 3 checklist items for ALL existing clients and insert new ones
DELETE FROM app_checklist_items WHERE fase_numero = 3;

-- Re-insert new phase 3 items for all existing clients
INSERT INTO app_checklist_items (cliente_id, fase_numero, texto, descricao, ator, tipo, ordem)
SELECT id, 3, 'Formulário do aplicativo', 'Preencha as informações do seu app para as lojas.', 'cliente', 'form', 1
FROM app_clientes;

INSERT INTO app_checklist_items (cliente_id, fase_numero, texto, descricao, ator, tipo, ordem)
SELECT id, 3, 'Desenvolvimento e Submissão do aplicativo pela Curseduca', 'A equipe Curseduca irá desenvolver e submeter o app nas lojas.', 'analista', 'check', 2
FROM app_clientes;
