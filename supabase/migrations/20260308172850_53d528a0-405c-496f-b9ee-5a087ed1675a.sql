...
-- Ensure plataforma column exists on app_checklist_items
ALTER TABLE app_checklist_items ADD COLUMN IF NOT EXISTS plataforma TEXT DEFAULT 'compartilhada';

-- Replace criar_fases_cliente with dual-platform support
...
BEGIN
  ...
  -- Checklist Fase 0 (compartilhada - igual para todos)
...
-- Replace recalcular_progresso_fase with per-platform progression
