...
-- Add store URL columns to app_clientes
ALTER TABLE app_clientes ADD COLUMN IF NOT EXISTS url_loja_google TEXT;
ALTER TABLE app_clientes ADD COLUMN IF NOT EXISTS url_loja_apple TEXT;

-- Update criar_fases_cliente to include publicacao_url items in fase 6
...
-- Backfill existing clients that have fase 6 but no publicacao_url checklist item
INSERT INTO app_checklist_items (cliente_id, fase_numero, texto, descricao, ator, tipo, ordem, plataforma)
SELECT f.cliente_id, 6,
  'Confirmar publicação e informar URL (' || CASE WHEN f.plataforma = 'google' THEN 'Google Play' ELSE 'App Store' END || ')',
  'Preencha a URL do app na loja para notificar o cliente. Prazo: 1 dia útil.',
  'analista', 'publicacao_url', 1, f.plataforma
FROM app_fases f
WHERE f.numero = 6
AND f.plataforma IN ('google', 'apple')
AND NOT EXISTS (
  SELECT 1 FROM app_checklist_items ci
  WHERE ci.cliente_id = f.cliente_id AND ci.fase_numero = 6 AND ci.tipo = 'publicacao_url' AND ci.plataforma = f.plataforma
);
